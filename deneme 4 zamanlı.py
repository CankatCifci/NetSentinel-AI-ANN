...
packet_times = []  # Gerçek zaman damgalarını tutacak liste
time_window = 30  # saniye olarak zaman aralığı
packet_threshold = 50  # eşik paket sayısı
...
while True:
    try:
        ...
        if processed_data:
            # Yeni paketler için gerçek zaman damgasını ekle
            for _ in packet_details:
                packet_times.append(current_time)

            # Zaman penceresinden eski kayıtları temizle
            cutoff_time = current_time - time_window
            packet_times = [t for t in packet_times if t >= cutoff_time]

            # Paket sayısı eşik kontrolü
            if len(packet_times) > packet_threshold:
                print(f"❗ Paket yoğunluğu anormal! Son {time_window} saniyede {len(packet_times)} paket.")

            sample_tensor = torch.tensor(processed_data, dtype=torch.float32)
            with torch.no_grad():
                output = model(sample_tensor)
                probs = torch.softmax(output, dim=1)  # Olasılık hesapla
                preds = (probs[:, 1] > 0.65).int().numpy()  # 0.65 eşik ile sınıflandır
